...
namespace ICICIMerchant.View
{
    ...
    /// </summary>
    public sealed partial class RegisterContactNumberView : Page
    {
        private NavigationHelper navigationHelper;
        private ObservableDictionary defaultViewModel = new ObservableDictionary();

        public RegisterContactNumberView()
        {
        ...
        private async void btnSubmit_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                postData = "tid=" + EncryptionProvider.Encrypt(((LoginModel)SuspensionManager.SessionState["loginModel"]).TID, DBHandler.key1, DBHandler.ivKey)
                       + "&phoneNumber=" + EncryptionProvider.Encrypt(txtAlternateNumber.Text, DBHandler.key1, DBHandler.ivKey);
            }
            catch (Exception)
            {
            }

            var alternateContactDetail = await Task.WhenAny(MakeHttpWebRequestPostCall.Generic_Service_Call(postData, DBHandler.url + DBHandler.alternatecontact_url_paddup,false));
            MessageDialog msgDlg = new MessageDialog("Result is " + alternateContactDetail.Result);
